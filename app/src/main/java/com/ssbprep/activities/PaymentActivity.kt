...
class PaymentActivity : AppCompatActivity(), PaymentResultListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // TESTING BUTTON: Bypasses actual payment for verification
        ...
        lifecycleScope.launch {
            viewModel.isPaid.collectLatest { isPaid ->
                if (isPaid && (binding.payNowButton.text == "Confirming..." || binding.testPayButton.text == "Processing...")) {
                    navigateToSuccess("Unlocked")
                }
            }
        }
    }

    private fun handleTestPayment() {
        binding.testPayButton.isEnabled = false
        binding.testPayButton.text = "Processing..."
        binding.payNowButton.isEnabled = false
        
        viewModel.completePayment("TEST_BYPASS_${System.currentTimeMillis()}") { success ->
            if (success) {
                navigateToSuccess("TEST_BYPASS")
            } else {
                binding.testPayButton.isEnabled = true
                binding.testPayButton.text = "Schedule Now (Testing)"
                binding.payNowButton.isEnabled = true
                Toast.makeText(this, "Test bypass failed to update Firestore", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun showLoading(isLoading: Boolean) {
        ...
        binding.payNowButton.text = if (isLoading) "Confirming..." else "Pay Now"
    }

    override fun onPaymentSuccess(razorpayPaymentId: String?) {
        showLoading(true)
        val txnId = razorpayPaymentId ?: "TXN_SUCCESS"
        
        viewModel.completePayment(txnId) { success ->
            if (success) {
                navigateToSuccess(txnId)
            } else {
                showLoading(false)
                Toast.makeText(this, "Payment successful but failed to update status. Please contact support.", Toast.LENGTH_LONG).show()
            }
        }
    }
